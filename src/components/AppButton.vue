...
<script>
/**
 * @props type @string 非必需 默認'primary' ['primary', 'secondary', 'floating', 'text', 'link', 'danger']
 * @props size @string 非必需 默認'md' ['sm', 'md', 'lg']
 * @props disabled @boolean 非必需 默認false
 */
export default {
  name: 'AppButton',
  props: {
  ...
  methods: {
    onClick() {
      this.$emit('click', 'i am params'); // 測試emit用
    },
  },
  computed: {
    cssClasses() {
